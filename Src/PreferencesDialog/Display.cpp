...
{
    m_selectColorView->setStyleSheet("QPushButton { background-color: #ffffff; }");
...
{
    connect(m_selectColorView, &QPushButton::clicked, this, &Display::pickBackgroundColorView);
    connect(m_selectColorSlideshow, &QPushButton::clicked, this, &Display::pickBackgroundColorSlideshow);
}
...
{
    QColor color = QColorDialog::getColor(bgColorView(), this);
    if (!color.isValid()) {
        return;
    }
...
{
    QColor color = QColorDialog::getColor(bgColorSlideshow(), this);
    if (!color.isValid()) {
        return;
    }
